...
run_RSOS <- function(L = 50, T_max = 5000, dim = 1) {
  if (dim == 1) {
    ...
    cat("\n[RSOS 1D] alpha(spec) =", signif(fit_a$alpha,4),
        " (slope=", signif(fit_a$slope,4), ", R2=", signif(fit_a$r2,4), ", k-window=", paste(signif(fit_a$window,4), collapse=".."), ")\n",
        "[RSOS 1D] beta =", signif(fit_b$beta,4), " (R2=", signif(fit_b$r2,4), ")\n",
        "[RSOS 1D] z = alpha/beta =", signif(z_est,4), "\n", sep="")
    ...
    fit_b <- estimate_beta(times, W)
    z_est <- estimate_z(fit_a$alpha, fit_b$beta)
    cat("\n[RSOS 2D] alpha(spec) =", signif(fit_a$alpha,4),
        " (slope=", signif(fit_a$slope,4), ", R2=", signif(fit_a$r2,4), ", k-window=", paste(signif(fit_a$window,4), collapse=".."), ")\n",
        "[RSOS 2D] beta =", signif(fit_b$beta,4), " (R2=", signif(fit_b$r2,4), ")\n",
        "[RSOS 2D] z = alpha/beta =", signif(z_est,4), "\n", sep="")
  }
}
